...
func TestMinGWProvider_DisplayName(t *testing.T) {
	p := NewMinGWProvider()
	if p.DisplayName() != "MinGW-w64 (GCC)" {
		t.Errorf("expected display name 'MinGW-w64 (GCC)', got '%s'", p.DisplayName())
	}
}

func TestMinGWProvider_Type(t *testing.T) {
	p := NewMinGWProvider()
	if p.Type() != models.CSDK {
		t.Errorf("expected type 'c', got '%s'", p.Type())
	}
}
...
func TestMinGWProvider_GetDefaultInstallPath(t *testing.T) {
	p := NewMinGWProvider()
	path := p.GetDefaultInstallPath("15.2.0")
	if !strings.Contains(path, "c") || !strings.Contains(path, "mingw") || !strings.Contains(path, "15.2.0") {
...
func TestMinGWProvider_Validate(t *testing.T) {
	...
	// Valid version
	if err := p.Validate("15.2.0"); err != nil {
